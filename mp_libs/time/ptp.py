...
class PtpMsgs(Enum):
    ...
    @classmethod
    def to_str(cls, msg: Union["PtpMsgs", int]) -> str:
        """Get the associated string for a given PtpMsgs enum value.

        Args:
            msg (PtpMsgs): PtpMsgs enum value (eg PtpMsgs.SYNC_REQ).
...
class PtpPacket():
    ...
    Msg must be one of the PtpMsgs values. Payload must be an int.
    """
    def __init__(self, msg: Union[PtpMsgs, int], payload: int) -> None:
    ...
    @property
    def msg(self) -> PtpMsgs:
        """Get packet message type.

        Returns:
            PtpMsgs: Packet message type.
        """
    ...
    def deserialize(cls, data: bytes) -> "PtpPacket":
        ...
        if not PtpMsgs.contains(ptp_tuple.msg):
...
def sync_req(transport: InterfaceProtocol, num_sync_cycles: int = 1, **kwargs) -> None:
    ...
    transport.send(PtpPacket(PtpMsgs.SYNC_REQ, num_sync_cycles).serialize(), **kwargs)
...
def sync_start(transport: InterfaceProtocol, **kwargs) -> int:
    ...
    t1 = rtc.now()
    transport.send(PtpPacket(PtpMsgs.SYNC_START, t1).serialize(), **kwargs)
    return t1
...
def delay_req(transport: InterfaceProtocol, **kwargs) -> int:
    ...
    t3 = rtc.now()
    transport.send(PtpPacket(PtpMsgs.DELAY_REQ, t3).serialize(), **kwargs)
    return t3
...
def delay_resp(transport: InterfaceProtocol, ts: int, **kwargs) -> None:
    ...
    transport.send(PtpPacket(PtpMsgs.DELAY_RESP, ts).serialize(), **kwargs)
...
def parse_msg(msg: bytes) -> Tuple[Union[PtpMsgs, int], int]:
    ...
    Returns:
        Tuple[Union[PtpMsgs, int], int]: (msg type, payload)
    """
...
def sequence_master(
    ...
    **kwargs
    ...
    while cycle_count < num_sync_cycles:
        ...
        _, t4 = wait_for_msg(PtpMsgs.DELAY_REQ, transport, msg_parser, timeout_ms, **kwargs)
...
def sequence_periph(
    ...
    **kwargs
    ...
    results = []
    while cycle_count < num_sync_cycles:
        t1, t2 = wait_for_msg(PtpMsgs.SYNC_START, transport, msg_parser, timeout_ms, **kwargs)
        t3 = delay_req(transport, **kwargs)
        t4, _ = wait_for_msg(PtpMsgs.DELAY_RESP, transport, msg_parser, timeout_ms, **kwargs)
...
def wait_for_msg(
    ...
    **kwargs
    ...
    Args:
        msg_type (Union[PtpMsgs, int]): Message to wait for.
    ...
    """
    logger.debug(f"Waiting for {PtpMsgs.to_str(msg_type)}")
    ...
    while not data_available:
        if timeout_ms is not None and time.ticks_diff(time.ticks_ms(), start) > timeout_ms:  # pylint: disable=no-member
            raise TimeoutError(f"Timed out waiting for PTP message: {PtpMsgs.to_str(msg_type)}.")
    ...
    logger.debug(f"Rx'ed msg: {PtpMsgs.to_str(msg_type)}, Payload: {payload}, TS: {rx_ts}")
